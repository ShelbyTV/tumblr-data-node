    ...
    self.dao.getUserInfo(uid, function(e, info){
      ...
      self.tumblr.getDashboard(params, info.access_token, info.access_token_secret, function(e, feed){
        if (e || !feed || !feed.response || !feed.response.posts.length){
          return console.log('http error or no feed returned', e); 
        } else {
          console.log(feed.response.posts.length, 'posts');
          console.log('first', feed.response.posts[0].id);
          console.log('last', feed.response.posts[feed.response.posts.length-1].id);
